...
def train_function(train_data, train_label):
    ...
    #inpput_weights is 960 * 100
    #training data.shape[0] is 184 coz there are 184 data points.
    #training data.shape[1] is 960
    ...
    while (epochs):
        for i in range(train_data.shape[0]):
            X1 = train_data[i] # x1 is 1 *960
            S1 = np.dot(X1, input_weights)  # s1 becomes 1 * 100
            X2 = sigmoid(S1) # pass to activation
            ...
            hidden_array = np.array(hidden_weights).T   # hideen array: 1x100
